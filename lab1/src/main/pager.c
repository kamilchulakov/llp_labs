...
page* allocate_page_typed(db_handler* db, page_type type) {
    ...
    pg->type = type;
    pg->next_page_id = db->db_file_header->first_collection_page_id;
    db->db_file_header->first_collection_page_id = pg->page_id;
...
page* get_free_collection_page(db_handler* handler) {
    if (handler->db_file_header->first_free_collection_page_id != -1) {
        ...
        handler->db_file_header->first_free_collection_page_id = pg->next_page_id;
        return pg;
    } else {
        page* pg = allocate_page_typed(handler, PAGE_COLLECTION);
        if (handler->db_file_header->first_collection_page_id == -1) {
            handler->db_file_header->first_collection_page_id = pg->page_id;
        }
        return pg;
    }
}
